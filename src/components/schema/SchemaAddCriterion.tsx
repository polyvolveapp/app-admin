...
import { SchemaActions, ReviewCategory } from "../../redux/schema"
import { InfoIcon } from "polyvolve-ui/lib/icons"
import { Error, Button, LoadButton } from "polyvolve-ui/lib"
import { modalStyle, cx, formStyle, Select, singleStyle, componentStyle, style } from "../../lib/reexports"
...
class SchemaAddCriterion extends React.Component<Props> {
  ...
  onSubmit = (values: FormData) => {
    ...
    this.props.schemaActions.createCriterionRequest({ category, name, description, type })
    this.props.onSubmit()
  }

  labelForValue = (value: string): string => value.charAt(0).toUpperCase() + value.slice(1)
    ...
    return (
      <Formik<FormData>
        ...
        render={({
          ...
          <form className={schemaStyle.schemaAddCriteiron} onSubmit={handleSubmit}>
            ...
            <div className={formStyle.formRowContainer}>
              <div className={formStyle.formRowGrid}>
                ...
                <div className={formStyle.formRowInputContainer}>
                  <Select
                    className={cx(style.selectMenu, componentStyle.select, style.widthInputSmall)}
                    classNamePrefix="pv"
                    placeholder={{ value: values.type, label: this.labelForValue(values.type) }}
                    value={{ value: values.type, label: this.labelForValue(values.type) }}
