...
import { ReviewCategory, SchemaActions, SchemaUpdateCategoryState, SchemaUpdateCriterionState, ReviewSchema } from "../../redux/schema";
...
class SchemaCategory extends React.Component<Props, State> {
  ...
  componentDidUpdate(oldProps: Props) {
    ...
    if (newProps.updateCategory.id === oldProps.data.id) {
      if (!oldProps.updateCategory.initialized && newProps.updateCategory.initialized) {
  ...
  toggleEdittingBasic = () => this.setState({ edittingBasic: !this.state.edittingBasic })
  ...
  onSubmitBasic = (values: FormDataBasic) => {
    ...
    schemaActions.updateCategoryRequest({ category: data, name: values.name, description: values.description })
  }
  ...
  onUp = () => {
    ...
    schemaActions.setOrderCategoryRequest({ schema, category: data, order: newOrder })
  }
  ...
  render(): JSX.Element {
    const { schema, schemaActions, addCriterion, data, updateCriterion } = this.props
    ...
    return (
      <div className={schemaStyle.category} key={`category-div-${data.id}`}>
        <div key={`category-title-bar-${data.id}`} className={cx(schemaStyle.categoryTitleBar, style.mb06)}>
          <h1 key={`category-title-name-${data.id}`}>{name}</h1>
          <div key={`category-title-iconBar-${data.id}`}>
            {data.order > 0 && <UpIcon size={{ width: 16, height: 16}} onClick={this.onUp} />}
            {data.order < schema.categories.length - 1 && <DownIcon size={{ width: 16, height: 16}} onClick={this.onDown} />}
            <RemoveIcon size={16} onClick={this.onClickToRemove} />
          </div>
        </div>
        <Line key={`category-separator-${data.id}`} className={cx(style.mb06)} />
        <Section size="full" key={`category-section-${data.id}`}>
          <Subsection size="full" key={`category-subsection-basic-${data.id}`} className={cx({ [style.isEditting]: edittingBasic })}>
            <SubsectionTitle key={`category-subsection-title-basic-${data.id}`} title="Basic" />
            <SubsectionContent key={`category-subsection-content-basic-${data.id}`} size="full">
              {!edittingBasic && <SectionInformation key={`category-section-information-basic-${data.id}`} items={[
                { name: "Name:", value: data.name },
                { name: "Description:", value: data.description },
              ]} />}
              {edittingBasic && <Formik<FormDataBasic>
                initialValues={{
                  name: data.name || "",
          ...
          <Subsection
            ...
            className={cx({ [style.isEditting]: edittingBasic }, schemaStyle.criteriaSubsection)}>
            <SubsectionTitle title="Criteria" key={`category-subsection-criteria-title-${data.id}`} />
            <SubsectionContent size="full" key={`category-subsection-criteria-content-${data.id}`} >
              {criteria.length === 0 && <p key={`category-subsection-criteria-p-${data.id}`} className={cx(style.alignCenter, style.mb1)}>
        ...
        <Button
          type="button"
          name="add-category"
          className={cx(formStyle.plusButton, schemaStyle.addCriterionButton, style.btnAlternative)}
          onClick={this.openAddCriterion}
          key={`category-button-add-category-${data.id}`}>+</Button>
        <SchemaAddCriterion
