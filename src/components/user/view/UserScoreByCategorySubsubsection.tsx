...
import { ReviewCategory } from "../../../redux/schema";
import { ScoreCategoryContainer } from "polyvolve-ui/lib/@types";
    ...
    <Subsubsection key={`subsubsection-${category.id}`}>
      ...
      <SubsubsectionContent
        ...
        <Collapser
          ...
          <div className={singleStyle.scoreContainer} key={`subsubsection-scoreContainer-${category.id}`}>
            <div key={`subsubsection-avg${category.id}`}>
              <p key={`subsubsection-stdDev-p-${category.id}`}>Avg</p>
              <p key={`subsubsection-stdDev-p2-${category.id}`}>{scores.overallScore.avg.toFixed(2)}</p>
            </div>
            <div key={`subsubsection-stdDev-${category.id}`}>
              <p key={`subsubsection-stdDev-p-${category.id}`}>Std Dev</p>
              <p key={`subsubsection-stdDev-p2-${category.id}`}>{scores.overallScore.stdDev.toFixed(2)}</p>
            </div>
          </div>
        </Collapser>
      </SubsubsectionContent>
